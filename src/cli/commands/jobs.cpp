...
static void do_allocs(BaseCLI& cli, const std::string& arg) {
    ...
    for (const auto& a : allocs) {
        ...
        // Remaining time (uses same logic as AllocationManager::remaining_minutes)
        std::string remaining = "-";
        if (!a.start_time.empty()) {
            // Parse start_time and compute elapsed
            struct tm tm_buf = {};
        ...
        // Use raw widths for the colored field (ANSI codes mess up fmt padding)
...
static void do_gpus(BaseCLI& cli, const std::string& arg) {
    ...
    for (const auto& r : resources) {
        ...
        std::string access_mark = accessible ? "" : " (no access)";
