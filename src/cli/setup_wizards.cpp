...
void TCCPCLI::run_register(const std::string& path_arg) {
    ...
    int entry_idx = prompt_choice("Entry point:", entry_options, 0);
    bool use_package = (entry_idx == 1);

    std::string entry_point;
    if (use_package) {
        entry_point = prompt_line("Package name", dirname);
    } else {
        ...
        entry_point = prompt_line("Main script", default_script);
    }
    ...
    yaml << "  " << job_name << ":\n";
    if (use_package) {
        yaml << "    package: " << entry_point << "\n";
    } else {
        yaml << "    script: " << entry_point << "\n";
    }
